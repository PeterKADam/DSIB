...
```{r}
US_births_1994_2003  |>  ggplot(aes(x = day_of_week, y = births, fill=day_of_week)) +
...
data <- data %>%
    mutate(datetype = as.factor(date)) %>%
    mutate(datetype = ifelse(day_of_week == "Sat", "Sat", ifelse(day_of_week == "Sun", "Sun", "Weekday")))

plot <- ggplot(data, aes(x = date, y = births, color = datetype)) +
    geom_point() +
    labs(
